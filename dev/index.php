<?php
   ...
   if ($total > 0) {
      ...
      $opcaoCartao = $lista['opcaoCartao'];
      $opcaoBoleto = $lista['opcaoBoleto'];

      $pixelCartao = $lista['pixelCartao'];
      $pixelBoleto = $lista['pixelBoleto'];

      //Tulio, 30/07/2019
      $urlAgradecimento = $lista['urlAgradecimento'];


      $usuarioID = $lista["usuarioID"];
      
      //lendo key do usuário
      $userQuery = mysqli_query($con, "SELECT * FROM admins WHERE adminID = $usuarioID LIMIT 1");
...
<html>
   <head>
      ...
      <link href="<?php echo SITE_URL ?>css/style.css?v=2.5'" rel="stylesheet">

       <!-- Global site tag (gtag.js) - Google Analytics -->
<script async src="https://www.googletagmanager.com/gtag/js?id=UA-140867063-2"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'UA-140867063-2');
</script>
    ...
    smartlook('init', '4df1573a193cfc48a5a884046ff3fd83df2dc561');
</script>
   </head>
   <body>
      <div class="container">
         <div class="pagina">
            ...
            <?php
             } //caso não exista url de agradecimento
             else{ //caso exista redireciona:
                echo "<script>window.location.href = '".$urlAgradecimento."';</script>";
             }
               }
               }
               } else {
            ...
            <form action="<?php echo SITE_URL .'?p='. $compraCodigo .'&pg=pagamento'; ?>" method="POST" id="frmDados">
                     ...
                     <div class="col-md-4">
                        <div class="form-group">
                           <div class="form-campo" id="div-cliente_cpf">
                              ...
                              <input required placeholder="CPF" type="text" id="cliente_cpf" name="cliente_cpf" maxlength="11" class="form-control floatlabel" value="<?php echo isset($_SESSION['cliente_cpf']) ? $_SESSION['cliente_cpf'] : ''; ?>" />
                           </div>
                           <span class="error"></span>
                        </div>
                     </div>
               ...
               <div class="pagina-conteudo">
                  <div class="campo-icone" id="div-endereco_cep">
                     ...
                     <input type="text" placeholder="CEP" required id="endereco_cep" name="endereco_cep" maxlength="8" class="form-control floatlabel" value="<?php echo isset($_SESSION['endereco_cep']) ? $_SESSION['endereco_cep'] : ''; ?>" /><a class="btn btn-success btn-cep" id="btn-cep">Pesquisar <i class="fa fa-search"></i></a>
                  </div>
                  <span class="error"></span>
               </div>
            ...
            <form action="" method="POST" id="frmPagar">
               ...
               <div class="pagina-conteudo pagamento-conteudo" id="boleto" <?php echo $opcaoCartao ? 'style="display: none"' : ''; ?>>
               ...
               <?php
               if ($opcaoCartao){
                     echo '<input type="hidden" name="tipo" id="tipo" value="cartao" />';
                  } else {
                     echo '<input type="hidden" name="tipo" id="tipo" value="boleto" />';
                     echo '<input type="hidden" name="cardNumber" id="cardNumber" value="" data-checkout="cardNumber">';
                  }
               ?>
      ...
      <?php
         if (isset($_GET['id'])) {
         	...
         	if (($pagamentoTipo == 'cartao' && $pixelCartao) ||
         		($pagamentoTipo == 'boleto' && $pixelBoleto)) { ?>
      ...
      <script type="text/javascript">
         ...
         window.mp_key       = '<?php echo $userMPKEY; ?>';
      </script>
