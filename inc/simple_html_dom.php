...
if (!function_exists('file_get_html2')) {
	function file_get_html2($url, $use_include_path = false, $context=null, $offset = 0, $maxLen=-1, $lowercase = true, $forceTagsClosed=true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT, $defaultSpanText=DEFAULT_SPAN_TEXT)
	{
		// Ensure maximum length is greater than zero
		if($maxLen <= 0) { $maxLen = MAX_FILE_SIZE; }

		// We DO force the tags to be terminated.
		$dom = new simple_html_dom2(null, $lowercase, $forceTagsClosed, $target_charset, $stripRN, $defaultBRText, $defaultSpanText);
...
if (!function_exists('str_get_html2')) {
	function str_get_html2($str, $lowercase=true, $forceTagsClosed=true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT, $defaultSpanText=DEFAULT_SPAN_TEXT)
	{
		$dom = new simple_html_dom2(null, $lowercase, $forceTagsClosed, $target_charset, $stripRN, $defaultBRText, $defaultSpanText);
...
// dump html dom tree
if (!function_exists('dump_html_tree2')) {
	function dump_html_tree2($node, $show_attr=true, $deep=0)
	{
		$node->dump($node);
	}
}
...
if (!class_exists('simple_html_dom_node2')) {
	class simple_html_dom_node2
	{
		/**
		 * Node type
		 *
	...
	class simple_html_dom2
	{
		...
		protected function prepare($str, $lowercase=true, $defaultBRText=DEFAULT_BR_TEXT, $defaultSpanText=DEFAULT_SPAN_TEXT)
		{
			...
			$this->root = new simple_html_dom_node2($this);
			$this->root->tag = 'root';
		...
		protected function parse()
		{
			while (true) {
				...
				$node = new simple_html_dom_node2($this);
				++$this->cursor;
		...
		function createElement($name, $value=null) {return @str_get_html2("<$name>$value</$name>")->first_child();}
		function createTextNode($value) {return @end(str_get_html2($value)->nodes);}
		function getElementById($id) {return $this->find("#$id", 0);}
		function getElementsById($id, $idx=null) {return $this->find("#$id", $idx);}
		function getElementByTagName($name) {return $this->find($name, 0);}
